...
## 一、认识数据
数据就是一个量化指标，一个量化事物的手段，具有客观性。所以他可以帮助我们发现事物的本质和认识事物的客观规律。无论我们是否触碰它，它已经发生就在那里。而面对这些数据的时候，产品经理要以一个求知者的角色，在数据中寻找答案，带着一个求证的态度分析解读数据，探索数据背后的隐含信息为我所用。

Web数据和App数据不同，下面是几个常用的数据指标：

* Web端：**访问量（Visits）、访客数 UV（Unique Visitor）、浏览量 PV（PageViews）、浏览页面的数量、访问量、访客数、跳出率、退出率、页面停留时长、网站停留时长、转化率**等。其中**浏览量**是网站的人气指标，是网站最基础的指标。转化率是网站最重要的指标，这是看访客有没有达成网站的目标有没有产出。
...
数据的获取工具有很多，免费的网站排名**Alexa**、免费的监测工具**Google Analytics**网站版和客户端版、**百度统计**、**友盟**等。一些数据直接可以获取，一些数据需要往项目中加入一些代码就可实现。
...
当然你也可以不使用第三方工具自己公司开发对应的数据统计工具，这样比较会耗时好处是可以避免公司的数据泄露的风险。
